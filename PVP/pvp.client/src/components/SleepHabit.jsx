...
const SleepHabitContainer = styled(Box)({
    position: 'fixed',
    top: 50,
    right: 0,
    height: '100%',
...
export default function SleepHabit() {
    ...
    useEffect(() => {
        const fetchCheckIns = async () => {
            try {
                const response = await fetch('https://localhost:7200/api/Habits/getuserhabitcheckins/7', {
    ...
    useEffect(() => {
        const fetchHabitId = async () => {
            try {
                const response = await fetch('https://localhost:7200/api/Habits/gethabituseridbyhabitidanduserid?habitId=7', {
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                });
                if (!response.ok) {
    ...
    const fetchNotes = async () => {
        try {
            const response = await fetch(`https://localhost:7200/api/Habits/notesbyuseridandhabitid?habitId=7`, {
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            });
            if (!response.ok) {
    ...
    return (
        <SleepHabitContainer style={{ transform: isVisible ? 'translateX(0)' : 'translateX(100%)', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)', overflow: 'auto', }}>
            ...
            <Box sx={{ padding: '16px' }}>
                ...
                <Box sx={{
                    ...
                    <Typography variant="body1" sx={{ fontSize: '1rem', color: '#333333' }}>Sleep {goal} {frequency.toLowerCase()} {time.toLowerCase()}</Typography>
                </Box>
                <Box
                    sx={{
                        display: 'flex',
