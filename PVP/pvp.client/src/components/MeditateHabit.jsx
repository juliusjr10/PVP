...
export default function MeditateHabit() {
    ...
    const fetchNotes = async () => {
        try {
            const response = await fetch(`https://localhost:7200/api/Habits/notesbyuseridandhabitid?habitId=2`, {
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            });
            if (!response.ok) {
    ...
    return (
        <MeditateHabitContainer style={{
            transform: isVisible ? 'translateX(0)' : 'translateX(100%)', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)', overflow: 'auto', }}>
