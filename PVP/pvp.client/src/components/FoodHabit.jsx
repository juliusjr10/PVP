...
export default function SmokingHabit() {
    ...
    const fetchNotes = async () => {
        try {
            const response = await fetch(`https://localhost:7200/api/Habits/notesbyuseridandhabitid?habitId=4`, {
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
            });
            if (!response.ok) {
    ...
    useEffect(() => {
        const fetchHabitId = async () => {
            try {
                const response = await fetch('https://localhost:7200/api/Habits/gethabituseridbyhabitidanduserid?habitId=4', {
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                });
                if (!response.ok) {
        ...
        <SmokingHabitContainer style={{ transform: isVisible ? 'translateX(0)' : 'translateX(100%)', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)', overflow: 'auto', }}>
            ...
            <Box sx={{ padding: '16px' }}>
                ...
                <Box sx={{
                    ...
                    <Typography variant="body1" sx={{ fontSize: '1rem', color: '#333333' }}>Eat healthy {goal} {frequency.toLowerCase()} {time.toLowerCase()}</Typography>
                </Box>
                <Box
                    sx={{
                        display: 'flex',
